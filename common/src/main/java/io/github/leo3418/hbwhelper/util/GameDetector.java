...
public class GameDetector {
    ...
    private static final String ORDINARY_START_TEXT = "§f§lBed Wars§r";

    /** Prompt client received in chat when a Bed Wars game in Rush Mode starts */
    private static final String RUSH_START_TEXT = "§f§lBed Wars Rush§r";

    /** Prompt client received in chat when a Bed Wars game in Ultimate Mode starts */
    private static final String ULTIMATE_START_TEXT = "§f§lBed Wars Ultimate§r";
    ...
    /**
     * Updates whether client is in a Bed Wars game when it transfers from one server to another.
     *
     * <p>The screen with dirt background, which might be accompanied by the text "Downloading
     * terrain", will show up when client transfers.
     *
     * <p>If client is leaving a Bed Wars game, fires a {@link ClientLeaveGameEvent} on this mod's
     * {@link EventManager#EVENT_BUS proprietary event bus}.
     *
     * <p>This method should be called whenever a {@link GuiOpenEvent} is fired.
     *
     ...
     *
     * <p>Because {@link GuiOpenEvent} will not be called when client disconnects, this method is
     * implemented to detect {@link ClientPlayerNetworkEvent.LoggedOutEvent}, which is fired when
     * client disconnects.
     *
     * <p>If client is leaving a Bed Wars game, fires a {@link ClientLeaveGameEvent} on this mod's
     * {@link EventManager#EVENT_BUS proprietary event bus}.
     *
     * <p>This method should be called whenever a {@link ClientPlayerNetworkEvent} is fired.
     *
    ...
    /**
     * Updates whether client is in a Bed Wars game by analyzing chat message client receives, and
     * fires corresponding events.
     *
     * <p>If a Bed Wars game starts, fires a {@link GameStartEvent} on this mod's {@link
     * EventManager#EVENT_BUS proprietary event bus}.
     *
     * <p>If client is rejoining a Bed Wars game, fires a {@link ClientRejoinGameEvent} on this mod's
     * {@link EventManager#EVENT_BUS proprietary event bus}.
     *
     * <p>If client is joining an in-progress game which it never played before (which is different
     * from rejoining a game), fires a {@link GameEvent} on this mod's {@link EventManager#EVENT_BUS
